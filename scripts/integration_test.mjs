...
(async () => {
  ...
  try {
    ...
    const tlRes = await fetch('http://127.0.0.1:4001/api/timelogs', {
    ...
    const atRes = await fetch('http://127.0.0.1:4001/api/attendance', {
      method: 'POST', headers: { 'Content-Type': 'application/json', cookie },
    ...
    const getTL = await fetch('http://127.0.0.1:4001/api/timelogs?userId=E-001', { headers: { cookie } });
    console.log('timelogs:', await getTL.json());

    const getAt = await fetch('http://127.0.0.1:4001/api/attendance?userId=E-001', { headers: { cookie } });
    console.log('attendance:', await getAt.json());

    // Now simulate clock-out update
    const endTime = new Date().toISOString();
    const putTL = await fetch(`http://127.0.0.1:4001/api/timelogs/${encodeURIComponent(tId)}`, {
    ...
    const putAt = await fetch(`http://127.0.0.1:4001/api/attendance/${encodeURIComponent(aId)}`, {
    ...
    const getTL2 = await fetch('http://127.0.0.1:4001/api/timelogs?userId=E-001', { headers: { cookie } });
    console.log('timelogs after put:', await getTL2.json());

    const getAt2 = await fetch('http://127.0.0.1:4001/api/attendance?userId=E-001', { headers: { cookie } });
