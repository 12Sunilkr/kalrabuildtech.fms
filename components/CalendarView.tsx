...
export const CalendarView: React.FC<CalendarViewProps> = ({
    tasks: tasksProp, orders, leaves, holidays, reminders, setReminders, attendanceData, currentUser, employees
}) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                ...
                const arr = ensureArray(payload && payload.tasks ? payload.tasks : payload);
                if (mounted) setTasks(arr);
                try {
                    const cr = await safeGet('/calendar');
                    const cp = extractPayload(cr);
    ...
    const [newReminderUserId, setNewReminderUserId] = useState<string | null>(currentUser.employeeId || (currentUser.role === 'ADMIN' ? 'ADMIN' : String(currentUser.id)) );
    ...
    const handleAddReminder = () => {
            if (newReminderTitle && selectedDate) {
                (async () => {
                    try {
                        const ownerId = newReminderUserId || currentUser.employeeId || String(currentUser.id);
                        const body = { userId: ownerId, date: formatDateKey(selectedDate), title: newReminderTitle };
                        await safePost('/reminders', body);
                        const r = await safeGet('/reminders', { cacheBust: true });
                        const p = extractPayload(r);
                        setReminders(ensureArray(p));
                        setNewReminderTitle('');
                    } catch (e) {
                        console.error('Add reminder failed', e && (e.stack || e.message || e));
    ...
    // Allow creating a quick task from calendar modal
    const [quickTaskTitle, setQuickTaskTitle] = useState('');
    const handleCreateQuickTask = async () => {
        if (!quickTaskTitle || !selectedDate) return;
        try {
            const payload = { title: quickTaskTitle, assignedTo: currentUser.employeeId || null, dueDate: formatDateKey(selectedDate) };
            await api.post('/tasks', payload);
            const r = await safeGet('/tasks');
            const p = extractPayload(r);
            setTasks(ensureArray(p && p.tasks ? p.tasks : p));
            setQuickTaskTitle('');
            setSelectedDate(null);
        } catch (e) { console.error('Create quick task failed', e && (e.stack || e.message || e)); alert('Failed to create task'); }
    };

    const handleDeleteReminder = (id: string) => {
        (async () => {
            try {
                await safeDelete(`/reminders/${id}`);
                const r = await safeGet('/reminders', { cacheBust: true });
                const p = extractPayload(r);
                setReminders(ensureArray(p));
            } catch (e) {
                console.error('Delete reminder failed', e && (e.stack || e.message || e));
    ...
    const getEventsForDate = (date: Date) => {
        ...
        // 1b. Server calendar events
        const calMatches = calendarEvents.filter(ev => {
            try {
                const sKey = ev.startTime ? formatDateKey(new Date(ev.startTime)) : null;
                const eKey = ev.endTime ? formatDateKey(new Date(ev.endTime)) : sKey;
                if (!sKey) return false;
                return dateKey >= sKey && dateKey <= (eKey || sKey);
            } catch (err) { return false; }
        });
        calMatches.forEach(ev => events.push({ type: 'CALENDAR', data: ev }));
        ...
        const uidCandidates = [] as string[];
        if (currentUser) {
            if (currentUser.employeeId) uidCandidates.push(currentUser.employeeId);
            if (currentUser.id !== undefined && currentUser.id !== null) uidCandidates.push(String(currentUser.id));
            // legacy admin literal (some older clients used 'ADMIN')
            uidCandidates.push('ADMIN');
        }
        const myReminders = reminders.filter(r => r.date === dateKey && (uidCandidates.includes(String(r.userId)) || currentUser.role === 'ADMIN'));
            ...
            {selectedDate && (
                <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh]">
                        ...
                        {/* Add Reminder Input */}
                        <div className="p-4 bg-yellow-50/50 border-b border-yellow-100 flex gap-2 items-center">
                        ...
                        <div className="p-4 border-b bg-white">
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Create Task for {format(selectedDate, 'MMM d')}</label>
                            <div className="flex gap-2">
                                <input value={quickTaskTitle} onChange={e => setQuickTaskTitle(e.target.value)} placeholder="Task title" className="flex-1 border p-2 rounded-xl" />
                                <button onClick={handleCreateQuickTask} className="px-4 py-2 bg-indigo-600 text-white rounded-xl">Create</button>
                            </div>
                        </div>
                            ...
                            {getEventsForDate(selectedDate).length === 0 ? (
                                ...
                                getEventsForDate(selectedDate).map((e, idx) => {
                                    switch (e.type) {
                                        ...
                                        case 'CALENDAR':
                                            const ce = e.data as any;
                                            return (
                                                <div key={idx} className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 flex gap-3 items-center">
                                                    <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-indigo-600 font-bold shrink-0 shadow-sm"><CalendarDays size={18} /></div>
                                                    <div className="flex-1">
                                                        <div className="text-xs font-bold text-indigo-400 uppercase">Event</div>
                                                        <div className="font-bold text-indigo-900">{ce.title}</div>
                                                        <div className="text-xs text-indigo-600/80">{ce.description}</div>
                                                    </div>
                                                </div>
                                            );
                                        default: return null;
