...
export const PerformanceReport: React.FC<PerformanceReportProps> = ({ employees, tasks, attendanceData, checklistInstances = [], checklistTemplates = [], timeLogs = {} }) => {
 ...
 const incompleteCombined = (selectedStats.total - selectedStats.completed) + (checklistStats.total - checklistStats.completed);
 combinedScore = totalCombined > 0 ? Math.round((incompleteCombined / totalCombined) * 100) : 0;
 }
 ...
 id: t.id,
 title: t.title,
 date: t.dueDate ? t.dueDate.split('T')[0] : 'No Date',
 type: 'TASK'
 })),
 ...overdueChecklists.map(c => {
 const template = checklistTemplates?.find(t => t.id === c.templateId);
 return {
 id: c.id,
 title: template ? template.taskName : 'Checklist Routine',
 date: c.date,
 type: 'ROUTINE'
 };
 })
 ...
 <div className="print-container bg-white rounded-3xl shadow-xl border border-slate-200 p-8 max-w-4xl mx-auto print:shadow-none print:border-none print:rounded-none print:max-w-none print:w-full">

 {/* ── REPORT HEADER ── */}
 <div className="flex items-center justify-between border-b-2 border-slate-800 pb-4 mb-5 print:pb-2.5">
 <div className="flex items-center gap-3">
 <img src={COMPANY_LOGO} alt="Company Logo"className="w-12 h-12 object-contain"/>
 <div>
 <h1 className="text-[22px] font-black text-slate-900 uppercase tracking-tight leading-none">KALRA BUILDTECH</h1>
 <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-[0.2em] mt-1">KPI PERFORMANCE REPORT</p>
 </div>
 </div>
 <div className="text-right">
 <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Report Date</div>
 <div className="text-xl font-black text-slate-800">{format(new Date(), 'dd MMM yyyy')}</div>
 {fromDate && toDate && (
 <div className="mt-1 text-[9px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">
 ...
 <div className="w-14 h-14 rounded-full bg-white border-2 border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
 {selectedEmployee.avatar
 ? <img src={selectedEmployee.avatar} className="w-full h-full object-cover"/>
 : <UserCircle size={36} className="text-slate-300"/>}
 </div>
 <div className="flex-1 min-w-0">
 <h2 className="text-xl font-black text-slate-900 leading-tight">{selectedEmployee.name}</h2>
 <div className="flex flex-wrap gap-1.5 mt-1.5">
 <span className="text-[10px] font-bold text-slate-500 bg-white px-2.5 py-0.5 rounded border border-slate-200">ID: {selectedEmployee.id}</span>
 <span className="text-[10px] font-bold text-slate-500 bg-white px-2.5 py-0.5 rounded border border-slate-200">{selectedEmployee.designation}</span>
 <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded border border-indigo-100">{selectedEmployee.department}</span>
 </div>
 </div>
 <div className="shrink-0 bg-indigo-600 px-5 py-3 rounded-xl text-center">
 <div className="text-[9px] font-extrabold text-indigo-200 uppercase tracking-widest">TASK SCORE</div>
 <div className="text-4xl font-black text-white leading-none">{combinedScore}%</div>
 </div>
 </div>
 ...
 <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.12em]">Time Analysis</h3>
 </div>
 {(() => {
 ...
 <span className="text-[8px] font-black text-blue-600 uppercase tracking-wide">Total Hours</span>
 </div>
 <div className="text-xl font-black text-blue-800 leading-none">{formatDecimalHours(analysis.totalHours)}</div>
 </div>
 ...
 <span className="text-[8px] font-black text-emerald-600 uppercase tracking-wide">Days Worked</span>
 </div>
 <div className="text-xl font-black text-emerald-800 leading-none">{analysis.workingDays}<span className="text-[9px] font-bold text-emerald-500 ml-1">d</span></div>
 </div>
 ...
 <span className="text-[8px] font-black text-amber-600 uppercase tracking-wide">Late Logins</span>
 </div>
 <div className="text-xl font-black text-amber-800 leading-none">{analysis.lateCount}<span className="text-[9px] font-bold text-amber-500 ml-1">d</span></div>
 </div>
 <div className={`bg-${analysis.tierColor}-50 border border-${analysis.tierColor}-100 rounded-xl p-3 `}>
 <div className="flex items-center gap-1.5 mb-2">
 ...
 <span className={`text-[8px] font-black text-${analysis.tierColor}-600 uppercase tracking-wide `}>Perf. Tier</span>
 </div>
 <div className={`text-sm font-black text-${analysis.tierColor}-800 leading-tight `}>{analysis.tier}</div>
 </div>
 <div className={`rounded-xl p-3 border ${analysis.missedCount > 0 ? 'bg-rose-50 border-rose-200' : 'bg-slate-50 border-slate-100'}`}>
 <div className="flex items-center gap-1.5 mb-2">
 ...
 <span className={`text-[8px] font-black uppercase tracking-wide ${analysis.missedCount > 0 ? 'text-rose-600' : 'text-slate-400'}`}>Audit Flags</span>
 </div>
 <div className={`text-xl font-black leading-none ${analysis.missedCount > 0 ? 'text-rose-700' : 'text-slate-300'}`}>{analysis.missedCount}</div>
 ...
 <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.12em]">
 Attendance
 ...
 <div className={`text-2xl font-black ${num} leading-none `}>{value}</div>
 <div className={`text-[8px] font-extrabold ${lbl} uppercase mt-1.5 tracking-wider `}>{label}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.12em]">Task Execution</h3>
 </div>
 ...
 <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.12em]">Routine Checklist</h3>
 <span className="ml-auto text-[8px] font-bold text-slate-400 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-full">
 ...
 <div className={`text-lg font-black leading-none ${num} `}>{fmt(value)}</div>
 <div className={`text-[8px] font-bold ${lbl} uppercase mt-1 tracking-wider `}>{label}</div>
 </div>
 ))}
 </div>
 <div>
 <div className="flex justify-between items-center mb-1">
 <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Pending Workload</span>
 <span className={`text-xs font-black ${checklistStats.pct <= 20 ? 'text-emerald-600' : checklistStats.pct <= 60 ? 'text-amber-600' : 'text-red-500'}`}>{checklistStats.pct}%</span>
 </div>
 <div className="h-2 bg-slate-200 rounded-full overflow-hidden .5">
 <div
 ...
 <div className="flex justify-between mt-1 text-[8px] text-slate-400 font-semibold">
 ...
 <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-[0.12em]">Overdue History</h3>
 </div>
 <div className="bg-white border border-red-100 rounded-xl overflow-hidden print:border-red-200">
 <table className="w-full text-left text-sm border-collapse table-fixed">
 <thead className="bg-red-50">
 <tr>
 <th className="py-2 px-3 text-[10px] font-black text-red-600 uppercase tracking-wider w-[20%]">Type</th>
 <th className="py-2 px-3 text-[10px] font-black text-red-600 uppercase tracking-wider w-[60%]">Task / Routine</th>
 <th className="py-2 px-3 text-[10px] font-black text-red-600 uppercase tracking-wider text-right w-[20%]">Due Date</th>
 </tr>
 </thead>
 ...
 <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded inline-block ${item.type === 'TASK' ? 'bg-orange-100 text-orange-700' : 'bg-teal-100 text-teal-700'}`}>
 {item.type}
 </span>
 </td>
 <td className="py-2 px-3 text-xs font-bold text-slate-700 align-top break-words pr-4">{item.title}</td>
 <td className="py-2 px-3 text-xs font-bold text-red-600 text-right align-top">{item.date}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="h-8 w-44 border-b border-slate-300"></div>
 </div>
 <div className="text-right">
 <div className="text-[9px] text-slate-300 uppercase font-bold tracking-widest">Generated by Kalra FMS</div>
 <div className="text-[8px] text-slate-300 font-medium mt-0.5">{format(new Date(), 'dd/MM/yyyy HH:mm')}</div>
 </div>
 </div>
 ...
 {/* Overdue History - Bulk Print */}
 {bulkOverdueItems.length > 0 && (
 <>
 <h3 className="text-lg font-black text-red-600 mb-4 mt-8 uppercase tracking-tight">Overdue Task & Routine History</h3>
 ...
 <th className="py-3 px-2 text-[11px] font-black text-red-600 uppercase tracking-wider w-[20%]">TYPE</th>
 <th className="py-3 px-2 text-[11px] font-black text-red-600 uppercase tracking-wider w-[60%]">TASK / ROUTINE</th>
 <th className="py-3 px-2 text-[11px] font-black text-red-600 uppercase tracking-wider text-right w-[20%]">DUE DATE</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-red-100">
 {bulkOverdueItems.map(item => (
 <tr key={`${item.type}-${item.id}`}>
 <td className="py-2.5 px-2 align-top">
 <span className={`text-[10px] font-bold uppercase tracking-tight px-2 py-0.5 rounded inline-block ${item.type === 'TASK' ? 'bg-orange-100 text-orange-700' : 'bg-teal-100 text-teal-700'}`}>
 {item.type}
 </span>
 </td>
 <td className="py-2.5 px-2 font-medium text-slate-700 text-xs align-top break-words pr-4">{item.title}</td>
 <td className="py-2.5 px-2 font-bold text-red-600 text-xs text-right align-top">{item.date}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <th className="py-3 px-2 text-[11px] font-black text-slate-500 uppercase tracking-wider w-[50%]">TASK TITLE</th>
 <th className="py-3 px-2 text-[11px] font-black text-slate-500 uppercase tracking-wider w-[25%]">ASSIGNED</th>
 <th className="py-3 px-2 text-[11px] font-black text-slate-500 uppercase tracking-wider w-[25%]">STATUS</th>
 </tr>
 </thead>
 ...
 return (
 <tr key={t.id}>
 <td className="py-2.5 px-2 font-medium text-slate-700 text-xs align-top break-words pr-4">{t.title}</td>
 <td className="py-2.5 px-2 text-slate-500 text-xs align-top">{t.createdDate}</td>
 <td className="py-2.5 px-2 align-top">
 <span className={`text-[10px] font-bold uppercase tracking-tight px-2 py-0.5 rounded inline-block ${getStatusBadgeColor(actualStatus)}`}>
 {actualStatus}
 </span>
 </td>
 </tr>
 ...
 {/* Footer - Bulk Print */}
 <div className="mt-12 pt-6 print:pt-4 border-t border-slate-200 flex justify-between items-end">
 ...
 const totalCombined = stats.total + checkStats.total;
 const incompleteCombined = (stats.total - stats.completed) + (checkStats.total - checkStats.completed);
 // Score is the percentage of incomplete work (lower is better, 0% = perfect)
 const score = totalCombined > 0 ? Math.round((incompleteCombined / totalCombined) * 100) : 0;
